...
CSV 기반 홍수 범람 위험 시각화에 Spark RDD 분석 결과와 지역별 기상 보정값을 결합해, 경남 행정동코드 단위의 위험 점수를 대시보드로 확인하는 프로젝트입니다.
...
PySpark가 있으면 Spark RDD로 실행되고, 없으면 동일한 RDD 연산 인터페이스의 로컬 어댑터로 실행됩니다.
...
현재 대시보드는 `risk_latest` 점수, 지역 순위, 강수 특성을 반영한 기상 보정값을 갱신하고 위험점수에 즉시 반영합니다.
